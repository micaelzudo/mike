...
def list_files() -> str:
    return os.listdir(store["working_folder"])

def read_file(file_name: str) -> str:
    with open(store["working_folder"] + "/" + file_name, "r") as file:
        return file.read()
    
def write_file(file_name: str, file_contents: str) -> bool:
    file = open(store["working_folder"] + "/" + file_name, "w")
    file.write(file_contents)
    file.close()
    return True
    
def append_to_file(file_name: str, file_contents: str) -> bool:
    file = open(store["working_folder"] + "/" + file_name, "a")
    file.write(file_contents)
    file.close()
    return True

def run_python_file(file_name: str) -> str:
    store["stdout"].mark_data_as_read()
    with open(store["working_folder"] + "/" + file_name) as file:
        try:
            exec(file.read())
        except Exception as e:
            print("Exception: " + str(e))
    execution_output = store["stdout"].get_unread_data()
    write_file(file_name + ".txt", execution_output)
    return execution_output

def create_python_file(file_name: str) -> str:
    from lms.agent import lms_autogen_coder
